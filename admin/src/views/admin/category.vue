    ...
    <div class="row">
      ...
      <div class="col-md-4">
        ...
        <table id="level2-table" class="table  table-bordered table-hover">
          ...
          <tbody>
          <tr v-for="category in level2" v-on:click="onClickLevel2(category)" v-bind:class="{'active' : category.id === active.id}">
            <td>{{category.id}}</td>
            <td>{{category.name}}</td>
            <td>{{category.sort}}</td>
            <td>
...
export default {
  ...
  methods: {
    ...
    all() {
      let _this = this;
      Loading.show();
      _this.$ajax.post(process.env.VUE_APP_SERVER + '/admin/admin/category/all').then((response)=>{
        Loading.hide();
